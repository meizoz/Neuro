...
class Neuron
{
	...
	void addTarget(int tar); //!< adds a target to the list of tagets @param tar index of the target in the brain
	void addPotential(double extPot); //!< adds external potential to the membrane potential @param extPot a value to add to the membrane potential
	std::vector<int> getTargets() const;//!< returns the Neuron's list of target (post synaptic neurons)
	
	bool update(int t, double extCur);
